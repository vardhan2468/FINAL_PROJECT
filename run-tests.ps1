...
try {
    ...
    Write-Host "  Email: $($r.email), Role: $($r.role)" -ForegroundColor White
...
try {
    $body = '{"name":"API Testing","description":"RESTful API testing","duration":6}'
    ...
    Write-Host "  Course ID: $($r.data.id), Name: $($r.data.name)" -ForegroundColor White
    Write-Host "  [PASS]" -ForegroundColor Green
...
try {
    $body = '{"name":"API Testing - Advanced","description":"Advanced API testing","duration":8}'
    $h = @{Authorization="Bearer $script:adminToken"}
    $r = Invoke-RestMethod -Uri "$baseUrl/api/courses/$script:courseId" -Method PUT -Body $body -ContentType "application/json" -Headers $h
    Write-Host "  Updated: $($r.data.name), Duration: $($r.data.duration) weeks" -ForegroundColor White
    Write-Host "  [PASS]" -ForegroundColor Green
    $passed++
} catch {
    Write-Host "  [FAIL] $_" -ForegroundColor Red
...
try {
    $body = '{"email":"bob.smith@example.com","password":"password123"}'
    $r = Invoke-RestMethod -Uri "$baseUrl/api/auth/login" -Method POST -Body $body -ContentType "application/json"
...
try {
    ...
    $student = $users.data | Where-Object { $_.email -eq "bob.smith@example.com" }
    ...
    if ($_ -like "*409*" -or $_ -like "*Conflict*") {
        Write-Host "  Already enrolled (expected)" -ForegroundColor Yellow
        Write-Host "  [PASS]" -ForegroundColor Green
        $passed++
    } else {
        Write-Host "  [FAIL] $_" -ForegroundColor Red
...
Write-Host "`nTEST 9: Student Cannot Add Course" -ForegroundColor Yellow
try {
    $body = '{"name":"Hacked Course","description":"Should fail","duration":1}'
    $h = @{Authorization="Bearer $script:studentToken"}
    $r = Invoke-RestMethod -Uri "$baseUrl/api/courses/add" -Method POST -Body $body -ContentType "application/json" -Headers $h
...
Write-Host "`nTEST 10: Student Cannot Update Course" -ForegroundColor Yellow
try {
    $body = '{"name":"Hacked","description":"Should fail","duration":99}'
    $h = @{Authorization="Bearer $script:studentToken"}
...
Write-Host "`nTEST 11: Student Cannot Delete Course" -ForegroundColor Yellow
try {
    $h = @{Authorization="Bearer $script:studentToken"}
...
Write-Host "`nTEST 12: No Token Access Denied" -ForegroundColor Yellow
try {
